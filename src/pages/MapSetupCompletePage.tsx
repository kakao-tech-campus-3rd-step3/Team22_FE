    ...
    <div className="relative h-full w-full">
      <div className="flex-1 overflow-y-auto pb-20">
        <div className="flex justify-center mb-6">
          <div ref={mapContainerRef} className="w-full h-[300px] rounded-xl" />
        </div>
        ...
        <div className="h-[130px] p-4 bg-zinc-800 rounded-lg flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar">
            {walkTimes.map((item) => (
              <div key={item.id} className="flex justify-center gap-4">
                <div>{item.day}요일</div>
                <div>{item.hour}시</div>
      ...
      <div className="absolute bottom-0 left-0 w-full z-10 ">
        <ButtonBar buttonText="설정 완료! " onButtonClick={handleComplete} isButtonDisable={false}>
